...
function get20s(testArr) {
  let newCentury = [];
  for (let i = 0; i < testArr.length; i++) {
    let art= testArr[i].years.split(' ');
    if (art[0] >= 1900){
      newCentury.push(testArr[i].name)
    }
  }
  return newCentury;
}
...
function addArtist(artists) {
  const newObj = {
    ...
    bio:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aliquam in consequat mauris. Maecenas sit amet velit magna. Maecenas ac ultricies est. Morbi tincidunt lacus arcu, nec placerat felis tincidunt at. Morbi commodo dui sit amet urna pellentesque auctor.",
  };
  artists.push(newObj);
  return artists;
}
...
function lotsOfArt(testArr) {
  const tonsPainted = [];
 for(let i = 0; i < testArr.length; i++){
  if(testArr[i].paintings > 100){
tonsPainted.push(testArr[i].name)
     }
   }
   return tonsPainted;
  }
